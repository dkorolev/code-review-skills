...
- Each skill documents one complete, copy-pasteable, multi-finding invocation in a fenced `sh` block. It calls the writer by absolute path (`"$SKILL_DIR/scripts/write_review.py"`) from the repository root — never after `cd`-ing into the skill, which would misplace a relative result path. `SKILL_DIR` is the absolute directory of the `SKILL.md` the agent loaded, shown in the example as the placeholder `<absolute directory of this SKILL.md>` and never as a guessed install path: a skill may be installed in the repository, in a harness's user-level skills directory, or wherever a runner put it. A script that is not found is re-resolved against that directory; one that is truly absent is reported, never recreated. The example puts every value in **single quotes**, where the shell expands nothing; the one escape is `'\''` for an apostrophe. Double-quoted values are forbidden: findings routinely quote code with backticks and `$`, and inside double quotes the shell would execute or expand them before the writer runs. `test_write_review.py` installs each skill outside the repository, fills in only that placeholder, and executes the documented block through a real shell from the repository root, and separately drives hostile values through the same quoting rule.
