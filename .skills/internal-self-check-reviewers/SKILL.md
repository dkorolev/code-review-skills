...
- **Script-owned JSON** — its executable `scripts/write_review.py` exists, is identical across all five reviewer skills, and the skill requires using it with scalar flags instead of hand-authoring JSON or generating serialization code during the review. Its documented example is a fenced `sh` block that sets `SKILL_DIR` to the placeholder `<absolute directory of this SKILL.md>` (never a guessed install path), calls the writer through `"$SKILL_DIR/scripts/write_review.py"` from the repository root (no `cd` into the skill), tells the agent to re-resolve a script that is not found and to report — never recreate — one that is absent, single-quotes every value with `'\''` as the only escape, forbids double-quoted values, and shows at least two findings including a backtick span, a `$`, a double quote, and an apostrophe.
